...
export async function signCancelFinkok(
  ...
  const soapEnvelope = `
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:can="http://facturacion.finkok.com/cancel" xmlns:s0="apps.services.soap.core.views">
   <soapenv:Header/>
   <soapenv:Body>
      <can:sign_cancel>
         <can:UUIDS>
            <s0:UUID UUID="${uuid}" Motivo="${motivo}" ${folioSustitucion ? `FolioSustitucion="${folioSustitucion}"` : ''}/>
         </can:UUIDS>
...
`.trim();
  ...
  // Verificar incidencias en el SOAP de Finkok
  const incidenciasContent = extractTag(responseText, 'Incidencias');
  if (incidenciasContent) {
    const mensajeIncidencia = extractTag(incidenciasContent, 'MensajeIncidencia');
    const codigoError = extractTag(incidenciasContent, 'CodigoError');
  ...
  // Des-escapar acuse si existe para extraer estatus del SAT
  const acuseMatch = responseText.match(/<s0:Acuse>([\s\S]*?)<\/s0:Acuse>/i);
  const acuseXml = acuseMatch ? acuseMatch[1].replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&') : responseText;

  const estatusMatch = acuseXml.match(/<EstatusUUID>([0-9]+)<\/EstatusUUID>/i) || responseText.match(/<s0:CodEstatus>([0-9]+)<\/s0:CodEstatus>/i);
  const estatus = estatusMatch ? estatusMatch[1].trim() : '201';

  // Códigos de error del SAT al cancelar
  // 201 = Solicitud recibida con éxito
  // 202 = Ya cancelada previamente
  if (estatus !== '201' && estatus !== '202') {
    const codEstatus = estatusMatch ? `[${estatus}] ` : '';
    throw new Error(`SAT rechazó la cancelación: ${codEstatus}`);
  }

  return {
    success: true,
    estatus: estatus,
    response: acuseXml || responseText
  };
}
